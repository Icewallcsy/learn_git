...
df_train.drop(df_train[(df_train["1stFlrSF"]>3000)&(df_train["SalePrice"]<250000)].index,inplace=True)
df_train.drop(df_train[(df_train["BsmtFinSF1"]>2000)&(df_train["SalePrice"]<200000)].index,inplace=True)
df_train.drop(df_train[(df_train["OpenPorchSF"]>500)].index,inplace=True)
df_train.drop(df_train[(df_train["EnclosedPorch"]>500)].index,inplace=True)
df_train.drop(df_train[(df_train["BsmtFinSF2"]>1400)].index,inplace=True)
...
df_all.drop(['MasVnrArea', 'PoolQC','MiscFeature','Alley'],axis=1,inplace=True)
...
df_all['TotalArea']=df_all['LotArea']+df_all['GrLivArea']+df_all['GarageArea']
...
#print np.any(np.isnan(train_x1))
#print np.any(np.isfinite(train_x1))
